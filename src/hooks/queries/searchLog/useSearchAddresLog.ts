...
export default function useSearchAddresLog(
    { page = 1, size = 12, direction = 'ASC', sido='서울', sigungu='송파구' }: Partial<SearchLogsAddressQueryParams> = {},
    queryOptions?: Partial<UseQueryOptions<SearchLogResponse, Error>>
  ) {
    return useQuery<SearchLogResponse, Error>({
      queryKey: searchLogKeys.address({page, size, direction, sido, sigungu}),
      queryFn: () => api.searchLog.getSearchAddresLogs({page, size, direction, sido, sigungu}),
      ...queryOptions,
    });
  }